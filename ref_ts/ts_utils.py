...
def generate_files(shortkey, ts, first_create):
    ...
    try:
        ...
        if first_create: #first time res creation
            hydroshare.add_resource_files(res.short_id, *files)
        else:
            for file in files: #page refresh / res update
                hydroshare.update_resource_file(res.short_id, file.name, file)
