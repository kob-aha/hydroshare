...
class ToolResourceType(AbstractMetaDataElement):
    ...
    def update(cls, element_id, **kwargs):
        ...
        if tool_res_type_res:
            if 'tool_res_type' in kwargs:
                tool_res_type_res.types_str = kwargs['tool_res_type']
...
class Fee(AbstractMetaDataElement):
    ...
    def create(cls, **kwargs):
        if 'content_object' in kwargs:
            ...
            fee = Fee.objects.create(value=kwargs.get('value'), description=kwargs.get('description'), content_object=metadata_type)
            return fee
        else:
